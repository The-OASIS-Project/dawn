...
int auth_db_apply_migrations(int current_version, const char *db_path) {
   ...
   /* v68: generic blobs table (original-file storage) + documents.original_blob_id.
    * Idempotent (IF NOT EXISTS + probe-guarded ALTER). */
   bool v68_ok = (current_version >= 68);
   if (current_version < 68) {
      if (auth_db_migrations_v68(s_db.db) == AUTH_DB_SUCCESS) {
         v68_ok = true;
      } else {
         OLOG_ERROR("auth_db: v68 migration (blobs table + original_blob_id) failed");
      }
   }

   /* v69: conversations.is_pinned + idx_conversations_pinned (pinned-conversations
    * WebUI list section). Idempotent (probe-guarded ALTER + IF NOT EXISTS index). */
   bool v69_ok = (current_version >= 69);
   if (current_version < 69) {
      if (auth_db_migrations_v69(s_db.db) == AUTH_DB_SUCCESS) {
         v69_ok = true;
      } else {
         OLOG_ERROR("auth_db: v69 migration (conversations.is_pinned) failed");
      }
   }

   /* v70: authoritative case-insensitive uniqueness for code_projects.name
    * (UNIQUE INDEX ... COLLATE NOCASE), matching the NOCASE application lookups.
    * Idempotent; a legacy case-variant collision is logged and skipped, not fatal. */
   bool v70_ok = (current_version >= 70);
   if (current_version < 70) {
      if (auth_db_migrations_v70(s_db.db) == AUTH_DB_SUCCESS) {
         v70_ok = true;
      } else {
         OLOG_ERROR("auth_db: v70 migration (code_projects NOCASE unique index) failed");
      }
   }
   ...
   /* v72: background-jobs foundation (conversations job columns + conversation_events).
    * Idempotent (column-exists probe + IF NOT EXISTS). */
   bool v72_ok = (current_version >= 72);
   if (current_version < 72) {
      if (auth_db_migrations_v72(s_db.db) == AUTH_DB_SUCCESS) {
         v72_ok = true;
      } else {
         OLOG_ERROR("auth_db: v72 migration (background-jobs schema) failed");
      }
   }
   ...
   /* v74: durable job goal (job_goal column + backfill from the first user
    * message).  Idempotent (column-exists probe short-circuits). */
   bool v74_ok = (current_version >= 74);
   if (current_version < 74) {
      if (auth_db_migrations_v74(s_db.db) == AUTH_DB_SUCCESS) {
         v74_ok = true;
      } else {
         OLOG_ERROR("auth_db: v74 migration (durable job goal) failed");
      }
   }
   ...
   /* v85: sessions.keepalive_enabled — session-keepalive (always-on) flag; 1 =
    * slide expires_at forward on heartbeat. Base SCHEMA_SQL carries the column
    * (literal DEFAULT 0 keeps SQLite's fast ADD COLUMN path); this ALTER back-fills
    * an existing DB. Gated `< 85` so it also runs on fresh installs (base added it →
    * duplicate-column expected + tolerated, mirroring the v84 pattern). */
   bool v85_ok = (current_version >= 85);
   if (current_version < 85) {
      rc = sqlite3_exec(
          s_db.db, "ALTER TABLE sessions ADD COLUMN keepalive_enabled INTEGER NOT NULL DEFAULT 0",
          NULL, NULL, &errmsg);
      bool duplicate = (errmsg && strstr(errmsg, "duplicate column"));
      if (rc != SQLITE_OK && !duplicate) {
         OLOG_ERROR("auth_db: v85 migration (keepalive_enabled) failed: %s",
                    errmsg ? errmsg : "unknown");
         v85_ok = false;
      } else {
         v85_ok = true;
