...
void initialize(PiperConfig &config) {
   ...
   if (config.useTashkeel) {
      ...
      spdlog::debug("Loading libtashkeel model from {}", config.tashkeelModelPath.value());
...
void loadModel(std::string modelPath, ModelSession &session, bool useCuda) {
   ...
   session.env = Ort::Env(OrtLoggingLevel::ORT_LOGGING_LEVEL_WARNING, instanceName.c_str());
   ...
   // Always append the CPU provider to ensure it is available
   //session.options.AppendExecutionProvider_CPU(OrtDeviceAllocator, OrtMemTypeDefault);

   // Slows down performance by ~2x
   session.options.SetIntraOpNumThreads(4);
...
void synthesize(std::vector<PhonemeId> &phonemeIds,
   ...
   std::vector<float> scales{ synthesisConfig.noiseScale, synthesisConfig.lengthScale,
                              synthesisConfig.noiseW };

   std::vector<Ort::Value> inputTensors;
   std::vector<int64_t> phonemeIdsShape{ 1, (int64_t)phonemeIds.size() };
   inputTensors.push_back(
       Ort::Value::CreateTensor<int64_t>(memoryInfo, phonemeIds.data(), phonemeIds.size(),
                                         phonemeIdsShape.data(), phonemeIdsShape.size()));

   std::vector<int64_t> phomemeIdLengthsShape{ (int64_t)phonemeIdLengths.size() };
   inputTensors.push_back(Ort::Value::CreateTensor<int64_t>(memoryInfo, phonemeIdLengths.data(),
   ...
   // Infer
   auto startTime = std::chrono::steady_clock::now();
   auto outputTensors = session.onnx.Run(Ort::RunOptions{ nullptr }, inputNames.data(),
   ...
   spdlog::debug("Synthesized {} second(s) of audio in {} second(s)", result.audioSeconds,
                 result.inferSeconds);
   ...
   for (int64_t i = 0; i < audioCount; i++) {
      int16_t intAudioValue = static_cast<int16_t>(
          std::clamp(audio[i] * audioScale, static_cast<float>(std::numeric_limits<int16_t>::min()),
      ...
      if (voice.synthesisConfig.phonemeSilenceSeconds) {
         ...
         for (auto sentencePhonemesIter = sentencePhonemes.begin();
            ...
            if (phonemeSilenceSeconds.count(currentPhoneme) > 0) {
               ...
               phraseSilenceSamples.push_back((std::size_t)(phonemeSilenceSeconds[currentPhoneme] *
                                                            voice.synthesisConfig.sampleRate *
                                                            voice.synthesisConfig.channels));
...
void textToWavFile(PiperConfig &config,
   ...
   writeWavHeader(synthesisConfig.sampleRate, synthesisConfig.sampleWidth, synthesisConfig.channels,
                  (int32_t)audioBuffer.size(), audioFile);
