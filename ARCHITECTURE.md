...
| **WebUI Audio** | Browser-side Opus streaming via WebCodecs + server-side decode/resample/ASR/TTS/encode pipeline. Also hosts **always-on voice mode** (server-side VAD + wake word, no browser AI) and the **visual rendering tool** (inline SVG/HTML/Chart.js diagrams). Browser **music** playback decodes Opus **off the main thread** in a Web Worker (→ transferred `MessagePort` → AudioWorklet) over a dedicated music socket, with **closed-loop flow control** (client reports buffer depth; server paces a ~2s lead) — the same dedicated flow-controlled transport Tier-1 satellites use. | [webui-audio.md](docs/arch/subsystems/webui-audio.md) |
...
| **Deep Research** (`src/tools/research_*`, `src/core/research_worker.c`) | **P0 shipped 2026-08-13.** Confirmation-gated `deep_research` tool spawns a detached **`research_worker`** (a background-job sibling — reuses `job_manager_*`, its own spawn path, NOT `job_worker.c`) that runs an **IterResearch** loop on a **bare, memory-free** session: each round resets history to a bounded digest + top open questions, plans sub-questions, does web `search`/`url_fetch` under a **read-only tool allowlist** (enforced at advertise AND execute), and records injection-gated evidence **claims** to a SQLite ledger (`research_runs/questions/claims/report_revisions`, schema v75). A **deterministic C stop-controller** ends it — natural-end reasons first (`concluded` via the agent's `research_conclude` signal → distinct-source `coverage` → `saturation` after N dry rounds that close nothing) then hard budgets as the backstop — so a run stops on diminishing returns or agent judgment instead of always grinding to the token ceiling. The report is a **view over `research_claims`** (compression can't lose evidence), filed to notes and summarized back to the originating chat (never `reinvoke_parent` — the untrusted-content boundary, §11). Off by default (`[research] enabled`). Remaining P1 (completeness critic) is design-only. | [DEEP_RESEARCH_DESIGN.md](docs/DEEP_RESEARCH_DESIGN.md) |
...
| Deep Research      | `[research]`                          | Master enable (opt-in; the `deep_research` tool refuses until on) + per-run budgets (rounds, tool calls, input-token ceiling, round-digest cap, sources-to-answer, saturation-stop dry-round count). A research run IS a background job, so it also obeys `[jobs]` caps. Two P1/debug knobs (`critic_max_rearm`, `capture_revisions`) are parsed + round-tripped but not yet enforced, so they are deliberately **not** surfaced in the panel |
